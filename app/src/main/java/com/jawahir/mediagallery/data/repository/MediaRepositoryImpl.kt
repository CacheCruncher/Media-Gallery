...
class MediaRepositoryImpl @Inject constructor(@ApplicationContext private val context: Context) :
            ...
            context.contentResolver.query(uri, projection, selection, selectionArgs, sortOrder)
                ?.use { cursor ->
                    ...
                    while (cursor.moveToNext()) {
                        val albumId =
                            cursor.getLong(cursor.getColumnIndexOrThrow(MediaStore.Files.FileColumns.BUCKET_ID))
                        val albumName =
    ...
    override fun getMediaByAlbum(albumId: Long): Flow<MediaResult<List<MediaModel>>> = flow {
        emit(MediaResult.Loading())
        try {
            val uri = MediaStore.Files.getContentUri("external")
            val projection = arrayOf(
            ...
            when (albumId) {
                ...
                -3L -> { // Fetch all videos
                    ...
                    selection =
                        "${MediaStore.Files.FileColumns.BUCKET_ID} = ? AND ${MediaStore.Files.FileColumns.SIZE} > 0"
                ...
                ?.use { cursor ->
                    ...
                    do {
                        try {
                            ...
                            val mediaType =
                                if (mediaTypeInt == MediaStore.Files.FileColumns.MEDIA_TYPE_VIDEO) MediaType.VIDEO else MediaType.IMAGE
